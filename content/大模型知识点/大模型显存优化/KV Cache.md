...
![KV Cache 自回归解码流程](/static/images/uploads/大模型显存优化/kv-cache-decode-flow.png)

图中把 KV Cache 分成两个阶段理解：Prefill 阶段一次性处理 prompt，把所有历史 token 的 K/V 写入缓存；Decode 阶段每步只对当前新 token 计算 \(q_t,k_t,v_t\)，把新的 K/V 追加到缓存尾部，再让当前查询 \(q_t\) 与全部历史 K/V 做注意力。注意 Q 不需要缓存，因为历史查询不会被后续 token 再次使用；缓存大小则会随层数、序列长度、KV 头数和 head_dim 线性增长。
